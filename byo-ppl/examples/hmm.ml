...
let _ =
  ...
  let data = Array.init 20 (fun i -> float i) |> Array.to_list in
  let dist = Distribution.split_list (infer ~n:20000 hmm data) in
...
let hmm prob data =
  let rec gen i states data =
    ...
    | pre_x :: _, y :: data ->
        let x =
          sample prob (gaussian ~mu:pre_x ~sigma:1.) ("x_" ^ Int.to_string i)
        in
        let () = observe prob (gaussian ~mu:x ~sigma:1.) y in
...
let _ =
  ...
  let data = Array.init 20 (fun i -> float i) |> Array.to_list in
  let dist = Distribution.split_list (infer ~n:10000 ~warmup:10000 hmm data) in
  let m_x = List.map Distribution.mean (List.rev dist) in
  List.iter2 (Format.printf "%f %f@.") data m_x
...
let _ =
  ...
  let data =
    Array.init 20 (fun i -> float i +. Random.float 1.) |> Array.to_list
  in
  let dist = Distribution.split_list (infer ~n:100_000 hmm data) in
  let m_x = List.map Distribution.mean (List.rev dist) in
  List.iter2 (Format.printf "%f %f@.") data m_x
...
let _ =
  Format.printf "@.-- HMM, CPS Particle Filter 2 --@.";
  let data =
    Array.init 100 (fun i -> float i +. Random.float 1.) |> Array.to_list
  in
  let dist = Distribution.split_list (infer hmm ~n:100 data) in
  let m_x = List.map Distribution.mean (List.rev dist) in
  List.iter2 (Format.printf "%f %f@.") data m_x
